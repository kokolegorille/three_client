...
export default class User {
    constructor(game, pos, heading) {
        ...
        this.root.rotation.set(0, heading, "XYZ")
    ...
    load() {
        ...
        loader.load(
            ...
            xhr => {
                console.log(xhr.loading, xhr.total)
            },
            err => {
                console.error(err)
            }
    ...
    set action(name) {
        ...
        const clip = this.animations ? 
            this.animations[name.toLowerCase()] :
            "idle"

        if (clip !== undefined) {
            const action = this.mixer ? this.mixer.clipAction(clip) : null
            if (name == "shot") {
